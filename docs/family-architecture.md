...
| **Corona** (academic upstream + Lux expansions) | `github.com/luxfi/corona` | Ring-LWE (`R_q`) | BLAKE3 (academic) / SHA-3 (Lux expansions) | no | academic | upstream + lifecycle DKG expansions in flight |
...
- **Corona** — the Ring-LWE production threshold (Lux's hardened fork
  of Corona). Always capitalised. Identifier form: `corona`.
- **Corona** — the academic Ring-LWE upstream plus Lux's lifecycle
  DKG expansions. Always capitalised. Identifier form: `corona`.

...
### Per-byte Shamir: dual-field design

v0.1 Shamir-shares the FIPS 204 seed byte-wise. The reference
implementation supports two fields:

- **GF(257)** for committees with n ≤ 256 (default for small
  validator sets, 64-byte wire share).
- **GF(q) = GF(8 380 417)** for committees with 256 < n ≤ q-1
  (mandatory for any large committee, 128-byte wire share).

Auto-selection logic in `ref/go/pkg/pulsarm/types.go::resolveField`
picks the narrowest field that fits. The canonical extreme committee
target is `TargetCommitteeSize = 1_111_111` (seven 1s); see
`spec/pulsar-m.tex` §"Large committees" for the formal scaling
theorem.

v0.2 will reshape this to polynomial Shamir over `R_q^k` matching
`spec/pulsar-m.tex` §3.3. The output is the same FIPS 204 signature
in all cases; the v0.2 reshape is required to make the
Lagrange-linearity sign path work without round-tripping through the
byte-sum representation.
...
Pulsar inherits Corona's IP posture. Pulsar-M needs a fresh
...
`spec/pulsar-m.tex` is **complete** as LaTeX source (1500+ lines
covering Front matter / Notation / Preliminaries / Algorithms /
Verification / Reshare / Identifiable abort / Security analysis /
Concrete parameters / Large committees / Encodings / System model /
Implementation considerations / Performance / Test vectors /
Complexity analysis / Deployment / Comparison / Patent claims /
Known limitations / Open questions). The companion files
`security-games.tex`, `system-model.tex`, and `parameters.tex` are
also complete. Building the PDF via `latexmk -pdf spec/pulsar-m.tex`
is on the active writing track once the bib resolution lands.
...
### Lattice-estimator parameter table

`spec/parameters.tex` now includes the classical + quantum
security-level table (numbers inherited verbatim from FIPS 204 §8
because Pulsar-M does not alter the underlying M-LWE parameters).
The Albrecht et al. lattice-estimator script
`estimator/params.sage` reproduces the table; the estimator commit
hash is pinned at submission in `estimator/README.md`.

# PULSAR-M — Committee-Threshold ML-DSA Certificate Layer

> **Status.** Architecture specification. This document defines the *certificate
> layer* of Pulsar-M: how committee-threshold ML-DSA is organized into epochs,
...
> NIST-MPTC submission spec (`spec/pulsar-m.tex`, `spec/parameters.tex`,
> `spec/system-model.tex`) and the proofs under `~/work/lux/proofs/pulsar-m/`.
>
...
> `ref/go/pkg/pulsar/`; everything else is a *Pulsar-M target* to build.
...
flawed "threshold post-quantum" marketing collapses them; Pulsar-M keeps them
apart on purpose.
...
- **Claim (B) — DKG/threshold *construction* soundness.** The distributed key
  ...
  proven in `~/work/lux/proofs/pulsar-m/`. This is **our** theorem, not NIST's.
...
*single-party* ML-DSA KeyGen/Sign/Verify. Pulsar-M's contribution is (A) that
...
So Pulsar-M does **not** push one global Shamir/FROST sharing across all N
...
## 2. What exists today vs. the Pulsar-M target

Grounded in `ref/go/pkg/pulsar/` (origin/main).

| Concern | Implemented today (file:line) | Pulsar-M target |
|---|---|---|
...
| Committee/epoch object model + on-chain registry | **NOT BUILT** — consensus currently wires a *placeholder* Pulsar-M verifier (`LLM.md`: "finality verify path replaces the placeholder Pulsar-M verifier") | This document's object model (§7–§9) + on-chain verifier (§13, Phase 5). |
| Package independence | **COUPLED** — `~/work/lux/threshold/pkg/thresholdd/` builds `corona.go` + `magnetar.go` + `pulsar.go` in **one** Go package | Pulsar-M in its **own** package (§16). |
| Permissionless dealerless safety | **Carried by Corona** (natively dealerless Ring-LWE) in the Quasar AND-mode dual-PQ cert | **Unchanged.** Pulsar-M is the FIPS-204-standard leg / defence-in-depth. |
...
Pulsar-M is defined for the BCC-proven scope only: **ML-DSA-65** and
**ML-DSA-87**. ML-DSA-44 has **no** Pulsar-M suite (see below).
...
anchored by the **Corona** leg in the AND-mode dual-PQ Quasar cert; Pulsar-M is
...
Pulsar-M committees at scale use Pulsar-TD/Pulsar-TEE genesis until the
...
This replaces the current **placeholder Pulsar-M verifier** in
...
```
pulsar-m:v1:dkg        DKG transcript hashing / finalization
pulsar-m:v1:share      share commitment / well-formedness binding
pulsar-m:v1:ticket     offline ticket commitment / id
pulsar-m:v1:sign       per-session signing transcript + message_digest base
pulsar-m:v1:bridge     message_digest domain for bridge/cross-chain certs
pulsar-m:v1:finality   message_digest domain for finality certs
pulsar-m:v1:upgrade    message_digest domain for upgrade/governance certs
```

`message_digest = SHAKE256(pulsar-m:v1:<kind> ‖ epoch_id ‖ committee_id ‖
...
**Pulsar-M must live in its own package** (its own module path under the pulsar
...
| **1** | **Package split** — extract Pulsar-M into its own package so Corona/Pulsar build independently (§16). | To build. |
...
| **5** | **On-chain verifier** — the cheap 5-check verify (§13), replacing the placeholder Pulsar-M verifier in `luxfi/consensus/protocol/quasar`; multi-committee policy enforcement (§12). | To build. |
...
- **(B) Construction** — Lux-authored. Online signing reduces to ML-DSA
  ...
  identifiable abort are proven in `~/work/lux/proofs/pulsar-m/`
...
- **Permissionless dealerless safety** — anchored by the **Corona** leg
  ...
  leg; Pulsar-M is the FIPS-204-standard, assumption-shared defence-in-depth
...
- **FIPS 204** — NIST, Module-Lattice-Based Digital Signature Standard (ML-DSA).
  *(The single-party standard whose **verifier** accepts Pulsar-M output — Claim
  A. NIST does not standardize threshold ML-DSA.)*

### Companion artifacts in this repo / ecosystem
- Algorithm-level spec: `spec/pulsar-m.tex`, `spec/parameters.tex`,
  `spec/system-model.tex`, `spec/security-games.tex`.
- Proofs: `~/work/lux/proofs/pulsar-m/` (LaTeX) + `~/work/lux/proofs/lean/Crypto/Pulsar_M/`.

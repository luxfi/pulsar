% Pulsar-M threat model -- included by pulsar-m.tex.
...
The Pulsar-M security analysis (\texttt{spec/security-games.tex})
...
Pulsar-M proactive resharing inherits the soundness argument from
...
argument lives in HIP-0077 \S``Identity''; the Pulsar-M layer
...
$\mathcal{A}$ cannot produce a valid Pulsar-M signature $\sigma^*$
...
A Pulsar-M signature $\sigma$ on message $\mu$ against group public
...
The MPTC submission documents how Pulsar-M handles each failure
...
adversary positioning}: Pulsar-M uses a beacon-derived per-reshare
quorum selection (HIP-0077 \S``reshare quorum'' F10 fix).
\item \textbf{Cross-mode hash-suite confusion}: NIST profile is
exclusively SHA-3 family; no BLAKE3 in Pulsar-M ever.
\end{itemize}

...
Takahashi, and Tibouchi (``Ringtail''~\cite{boschini2024ringtail}), retargeted
...
The companion scheme Corona (R-LWE)~\cite{boschini2024ringtail} is a
...
implementation~\cite{boschini2024ringtail} to the module
ring $\Rq^k$.
...
Identical to the BDLOP18~\cite{boschini2024ringtail} commitment-hiding
...
long-term FIPS~204 ML-DSA-65 identity key
(\S\ref{sec:identity} \texttt{IdentityKey}; CR-6 closure since
v1.0.6). Complaint aggregation rules are specified per the system
model (\S\ref{sec:system-model}).
...
All complaints are signed under the accuser's long-term FIPS~204
ML-DSA-65 identity key (\texttt{IdentityKey} per
\S\ref{sec:identity}; CR-6 closure since v1.0.6). The system model's
slashing layer (\S\ref{sec:system-model}) consumes complaints; a
quorum of distinct accusers against the same accused is a
...
The tag width is pinned at the DD-008 encoding-freeze gate.
...
specified in \S\ref{sec:impl-ct}. The jasmin-ct gate on the
threshold layer (\texttt{round1.jazz}, \texttt{round2.jazz},
\texttt{combine.jazz}) is blocking at 3/3 CT-clean; libjade
ML-DSA-65 single-party sign is advisory under tracked issue \#2
(write-up: \texttt{ct/jasmin-ct-libjade.md}). Statistical timing
runs through \texttt{dudect} at $10^9$ samples per target in
\texttt{scripts/nightly.sh}, with results checked into
\texttt{ct/dudect/results/}. Physical side channels (power
analysis, EM, fault injection, cache timing) belong to the
deployed-binary layer; the algorithm-correctness chain in
\S\ref{sec:knownlimits} is independent.
...
$\Sign$ Round-2 contribution --- is a deployed-binary concern handled
by the operator runbook at \texttt{docs/deployment.md}.
...
\paragraph{Pulsar (R-LWE), Class~S~\cite{boschini2024ringtail}.}
...
Ringtail~\cite{boschini2024ringtail} construction with the beacon-quorum
strengthening of DD-006, both of which are explicitly placed into
the public domain by this submission.

\section{Closure record and round-1 scope boundary}
\label{sec:knownlimits}
Mirror of \texttt{BLOCKERS.md} (closed-finding registry; Open: none).
The 4-red-agent + 1-scientist adversarial audit (2026-05) closed
every cryptographic finding it surfaced; each closure cites commit
and tag in \texttt{BLOCKERS.md} at the repository root.

\paragraph{Cryptographic closures against the audit.}

\begin{itemize}[leftmargin=*, itemsep=0pt]
\item \textbf{DKG commit-opening (CR-6)} [CLOSED in v0.2.0]: The
unopened \texttt{cSHAKE256(c\_i $\Vert$ blind\_i)} commit was
dropped (Path A). The reference implementation realises the
Shamir+sum protocol with no separate commit-and-open layer;
binding comes from Round-2 envelope-digest agreement over the
ordered Round-1 broadcast set (\S\ref{sec:dkg}).
\item \textbf{Sign-round session-key derivation (CR-7)} [CLOSED in
v0.2.0]: \texttt{deriveMACKey} was deleted. The threshold signer
consumes a per-pair ephemeral session key established before
Round~1 via authenticated ML-KEM-768 key agreement, where each
party encapsulates a fresh shared secret to the peer's long-term
ML-KEM-768 identity public key and signs the ciphertext with their
long-term ML-DSA-65 identity key. The final session key mixes both
parties' contributions through HKDF-SHA3-256 over a
canonical-ordered NodeID pair (\S\ref{sec:sign} and
\S\ref{sec:system-model}).
...
$\textsc{DKGShareEnvelope} = (\mathit{KEMCT}, \mathit{Sealed})$
where $\mathit{KEMCT}$ is the ML-KEM-768 ciphertext (1088 B) and
$\mathit{Sealed}$ is the 128-byte sealed payload carrying
$(\mathit{share} \| \mathit{contribution} \| \mathit{tag})$ XORed
with a cSHAKE256 stream keyed by the KEM-derived envelope key
(\S\ref{sec:dkg}).
\item \textbf{Adaptive corruption proof.}
\Cref{thm:reshare-pkinv} is proved against the static-corruption
model (the MPTC requirement). The adaptive-corruption variant
(Game ADAPT) ports the static reduction through the
chain-corruption simulator and is targeted for the v0.2 audit
cycle.
\item \textbf{Cross-domain isolation between Pulsar (M-LWE) and
Corona (R-LWE).} Both inhabit the same algebraic-lattice hardness
family. Deployments seeking lattice-family diversity compose them
with a hash-based (SLH-DSA) signer at the consensus envelope;
Magnetar is the Tier-3 research track for this composition.
\item \textbf{Verify constant-time.} \Cref{sec:verify} routes
verification through \texttt{cloudflare/circl}'s FIPS~204
verifier, documented constant-time. The \texttt{ct/dudect/}
harness pins the $10^9$-sample run in
\texttt{scripts/nightly.sh}; results check into
\texttt{ct/dudect/results/}.
\item \textbf{$\Sign$ Round-2 byte-equality (PULSAR-V03-1)}
[CLOSED in v1.0.20, commit \texttt{023a3ed}]: the spurious
post-sample forward-NTT on $A$ in \texttt{deriveKeyMaterial} is
dropped; $A$ is sampled directly into the NTT representation per
FIPS~204 \S3.5 Algorithm~32. Regression guards in v1.0.21
(\texttt{267ec04}) and v1.0.22 (\texttt{29094a7}).
\end{itemize}

\paragraph{Round-1 scope boundary.}

\begin{itemize}[leftmargin=*, itemsep=0pt]
\item \textbf{Network model.} Round-1 assumes synchronous broadcast
within the round window. Asynchronous identifiable-abort
attribution routes through a separate consensus-layer
accountability artifact.
\item \textbf{HSM integration.} A deployed-binary concern handled
by the operator hardening checklist in
\texttt{docs/deployment.md}.
\item \textbf{Physical side channels} (power, EM, fault, cache
timing): handled at the deployed-binary layer; the
algorithm-correctness chain in the EasyCrypt refinement proof is
unchanged.
\item \textbf{Cross-message restart hybrid.}
\Cref{thm:restart-non-leak} proves restart non-leakage for a
single $(\mathit{pk}, \mu)$. The v0.2 wire shape ports the restart
semantics through the same MPTC submission framework.
\item \textbf{Lattice estimator parameter pins.} Concrete
bit-security estimates from the lattice estimator
(\texttt{estimator/}) are pinned at submission; the underlying
estimator commit hash is recorded in \texttt{estimator/README.md}.
\end{itemize}
...
\cite{boschini2024ringtail} paper's adaptive variant uses a complexity

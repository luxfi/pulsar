% Pulsar NIST MPTC category targeting -- included by pulsar.tex.
...
\subsection{Pulsar target: Class N1 + N4}
...
Pulsar aims for \textbf{N1 (threshold ML-DSA signing) + N4
(distributed ML-DSA keygen)}. The headline interchangeability claim:

\begin{quote}
A signature $\sigma$ produced by a Pulsar threshold ceremony on
...
If we can prove and demonstrate this, Pulsar is a Class N
candidate. If the output is not bit-for-bit FIPS~204 (e.g.\ encoding
differs in any field), Pulsar falls back to Class S --- still
...
For Pulsar to claim Class N1, the threshold-aggregated signature
...
2-round structure handles this for R-LWE; Pulsar needs to
...
\midrule
Technical Specification & PDF (LaTeX) & \texttt{spec/pulsar.pdf} \\
...
\item Closure-record appendix (\texttt{BLOCKERS.md}) so reviewers
see every closed finding with commit + tag.
\end{itemize}
...
Pulsar will ship parameter sets matching ML-DSA-65 (Category 3)
...
framing for Corona-derivatives unless and until NIST blesses
Corona-shaped outputs as a primitive.

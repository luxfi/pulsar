% Pulsar design decisions log -- included by pulsar.tex.
...
\paragraph{Decision.}
Pulsar aims for Class N1 + N4 (NIST MPTC IR 8214C):
...
OpenSSL 3.0) consume Pulsar certs without code changes. This is
...
\paragraph{Decision.}
Pulsar's NIST profile uses exclusively the FIPS~202 + SP 800-185
...
Pulsar is NIST-track and shouldn't ship a non-NIST default.
...
\paragraph{Decision.}
Pulsar operates over $\Rq^k$ (module of polynomials) with the same
$(q, k, \ell, \eta, \beta, \omega, \ldots)$ parameter set as
ML-DSA-65. Not $\Rq$ (single polynomial) as Pulsar / Corona use.
...
\paragraph{Alternative.}
Trusted-dealer DKG (academic Corona). Rejected: incompatible with
open public chains (HIP-0077 \S``Pulsar vs Corona'').
...
Pulsar's reshare selects via the chain's randomness beacon at the
reshare epoch boundary.

\paragraph{Alternative.}
Deterministic quorum, accepting the resulting mobile-adversary
leakage as a documented deployment constraint. Rejected for the
MPTC submission.

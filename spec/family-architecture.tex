...
% pulsar.tex. Mirrors docs/family-architecture.md (now deleted;
...
\midrule
\textbf{Pulsar} (was Pulsar / pulsar) &
...
\textbf{Annulus} (archived; was \texttt{Nasua}; tracks Corona upstream) &
  \texttt{github.com/luxfi/annulus} &
  Ring-LWE ($\Rq$) &
  BLAKE3 (academic profile) &
  no &
  academic-only &
  \emph{archived}; research-frozen; production descendants are Pulsar and Corona \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{Lineage.} Annulus is the archived academic root of the
family. The Lux fork was originally released under the code-name
\texttt{Nasua}; the formal-paper name \texttt{Annulus} replaced it
at the academic-archive cut. Annulus tracks the Corona primitive
of Boschini et~al.~\cite{boschini2024ringtail}, which builds on
the Raccoon Fiat--Shamir-without-rejection template~\cite{raccoon2024}.
Annulus is \emph{not enabled} in Lux production and the repository
is archived (no further academic-level changes).

The production descendants of Annulus are:
\begin{itemize}[leftmargin=*, itemsep=0pt]
\item \textbf{Corona} --- the Lux R-LWE production fork of Annulus.
Switches the hash family to SHA-3 / SP~800-185, adds identifiable
abort, adds proactive resharing with beacon-randomised quorum
selection.
\item \textbf{Pulsar} --- the Module-LWE / FIPS~204-compatible
expansion. Lifts the same two-round protocol from $\Rq$ to $\Rq^k$
so the threshold output is byte-equal to a single-party FIPS~204
ML-DSA signature.
\end{itemize}
...
\begin{itemize}[leftmargin=*, itemsep=0pt]
\item \textbf{Pulsar} --- rotating neutron star; the M-LWE
production variant.
\item \textbf{Corona} --- ring of light around a star; the R-LWE
production variant.
\item \textbf{Annulus} --- ring-shaped region (the ``ring of fire''
of an annular eclipse); the archived academic root. The
implementation repository was originally named after the ring-tailed
coati \emph{Nasua nasua}; the formal-paper name Annulus is the
...
(\texttt{Pulsar.R-BLAKE3}, upstream Corona) and are explicitly
...
reshare path, KAT suite, deployed code. Pulsar has a complete spec
(\texttt{spec/pulsar.tex} plus \texttt{parameters.tex},
...
it in Pulsar. Soundness arguments compose; reviewers don't have to
redo work.
...
hardened fork of Corona). Always capitalised. Identifier form:
\texttt{corona}.
\item \textbf{Corona} --- the academic Ring-LWE upstream plus Lux's
...
\begin{itemize}[leftmargin=*, itemsep=0pt]
\item \textbf{Ringtail} is the published Ring-LWE construction of
Boschini, Kaviani, Lai, Malavolta, Takahashi and Tibouchi
(\cite{boschini2024ringtail}). It is their name for their scheme and is
used whenever the paper is meant. \textbf{Corona} is Lux's
implementation of it; \textbf{Pulsar} retargets the same two-round
structure to Module-LWE for FIPS~204.
...
\begin{quote}
Submit BOTH. Pulsar leads (Class N1+N4, FIPS~204
...
\texttt{pulsar} and \texttt{pulsar} at the same SHA discipline.
...
\item[$\boxtimes$] Pulsar repo bootstrap (\texttt{pulsar/})
\item[$\square$] Pulsar Module-LWE algorithm body in spec (in
flight via scientist agent)
\item[$\square$] Pulsar reference implementation
\item[$\square$] Pulsar FIPS~204 cross-validation (output
interchangeability proof)
\item[$\square$] Pulsar \texttt{mptc-preview-2026} tag
...
Groth16/BN254 with a PQ-secure SNARK. Pulsar's per-validator

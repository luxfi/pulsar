...
\textbf{Nasua} (early academic prototype; tracks Corona upstream) &
  \texttt{github.com/luxfi/nasua} &
  Ring-LWE ($\Rq$) &
  BLAKE3 (academic profile) &
  no &
  academic-only &
  \emph{not enabled}; research artifact \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{Lineage.} Nasua is the early academic prototype (forks
the Corona primitive of Boschini et~al.~\cite{boschini2024corona},
which builds on the Raccoon Fiat--Shamir-without-rejection
template~\cite{raccoon2024}). Corona is the Lux production-hardened
R-LWE fork of Nasua: SHA-3 hash family, identifiable abort,
...
production variant.
\item \textbf{Nasua} --- the ring-tailed coati \emph{Nasua nasua};
the academic-only research prototype that forks the Corona
upstream verbatim. Single-word brand, repo-name aligned.
\end{itemize}

\paragraph{Reserved names.} \texttt{Annulus} (mathematical
ring-shaped region) is held in reserve for a future next-generation
variant. It is \emph{not} the name of any current artifact; do not
reference it in shipping code, wire formats, or specifications
until a concrete future variant claims it.

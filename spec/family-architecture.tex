...
\begin{quote}
\textbf{Pulsar} is one name for a family of lattice-based threshold
signature schemes. Variants share the same protocol skeleton
(2-round threshold sign, Pedersen DKG with proper hiding, proactive
...
Every consumer that wires ``the Pulsar family'' into a downstream
system points here for the variant table.
...
\textbf{Pulsar} (was Pulsar.M / pulsar-m) &
  ...
  Class N1 + N4 &
  this repo; reference complete, KAT-pinned \\
\textbf{Corona} (was Pulsar.R / luxfi/pulsar) &
  ...
  Class S1 + S4 &
  production R-LWE fork, lifecycle DKG complete \\
\textbf{Corona} (academic upstream + Lux expansions) &
  \texttt{github.com/luxfi/corona} &
  Ring-LWE ($\Rq$) &
  BLAKE3 (academic) / SHA-3 (Lux expansions) &
  no &
  academic &
  upstream + lifecycle DKG expansions in flight \\
\bottomrule
\end{tabular}
\end{center}

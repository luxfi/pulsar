...
// TEEQuoter is the runtime hook the aggregator uses to obtain a
// hardware-enclave attestation quote covering the Combine()
// reconstruction window. The aggregator wires a Quoter at startup
// based on which platform the binary is running on:
//
//	Intel SGX (DCAP)     —  sgxquoter.New() in pulsar/tee/sgx
//	AMD SEV-SNP          —  snpguest binding in pulsar/tee/sev
//	Intel TDX            —  tdx-quote in pulsar/tee/tdx
//	ARM CCA              —  cca-token in pulsar/tee/cca
//	AWS Nitro Enclaves   —  nsm-driver in pulsar/tee/nitro
//	Software-only (test) —  TestQuoter (never accept in production)
//
// Quote MUST embed reportData verbatim in the platform-specific
// REPORT_DATA / USER_DATA slot. Platform implementations that
// cannot bind the full 64 bytes MUST hash reportData to fit
// (SGX has a 64-byte slot, SEV-SNP has a 64-byte slot, TDX has a
// 64-byte slot — none of them need truncation for this caller).
...
// CombineReportData builds the 64-byte report-data binding that
// goes into the TEE quote. It is a pure function of the public
// signing-event inputs — no secret material participates.
//
// Domain separation: the "pulsar-combine-window-v1" suffix scopes
// this binding to the threshold-Combine reconstruction window, so
// the same enclave attesting to (say) a DKG transcript or a TEE-
// guarded key-resharing cannot have its quote replayed as proof of
// a Combine reconstruction.
//
// The output is exactly 64 bytes so it drops into the SGX/TDX/SEV
// 64-byte REPORT_DATA slot without truncation or hashing.
func CombineReportData(sessionID [16]byte, attempt uint32, groupPubkey *PublicKey, message []byte) []byte {
	parts := [][]byte{
		sessionID[:],
		{byte(attempt >> 24), byte(attempt >> 16), byte(attempt >> 8), byte(attempt)},
	}
	if groupPubkey != nil {
		parts = append(parts, groupPubkey.Bytes)
	}
	parts = append(parts, message)
	parts = append(parts, []byte("pulsar-combine-window-v1"))
	var buf []byte
	for _, p := range parts {
		buf = append(buf, p...)
	}
	return cshake256(buf, 64, tagCombineWindow)
}

// CombineWithAttestation runs Combine() and additionally requests
// a TEE attestation quote covering the reconstruction window. The
// returned Signature carries Attestation populated with the
// platform's evidence so relying parties that distrust the
// aggregator host can verify that reconstruction happened inside
// a measured enclave.
//
// When quoter is nil the function falls through to plain Combine();
// the returned Signature has Attestation == nil and the caller is
// responsible for documenting that the deployment's trust model
// includes the aggregator host.
//
// The quoter is invoked AFTER the FIPS 204 sign call succeeds and
// BEFORE the secret-material zeroisation in Combine(). Any error
// from the quoter discards the just-computed signature and returns
// the quoter error verbatim — we never emit a signature without
// attestation when the operator wired one in.
func CombineWithAttestation(
	params *Params,
	...
	sig, err := Combine(params, groupPubkey, message, ctx, randomized, sessionID, attempt, quorum, threshold, round1, round2, allShares)
	if err != nil {
		return nil, err
	}
	if quoter == nil {
	...
	reportData := CombineReportData(sessionID, attempt, groupPubkey, message)
	quote, qerr := quoter.Quote(reportData)
	if qerr != nil {
		return nil, qerr
	}
	sig.Attestation = &TEEAttestation{
		Platform:           quoter.Platform(),
		Quote:              quote,
		ReportData:         reportData,
		EnclaveMeasurement: quoter.EnclaveMeasurement(),
	}
	return sig, nil
}

...
//	      -> pulsarm.VerifyCtx(params, pk, M, ctx, sig)            // wrapper
//
...
//     and (b) pulsarm.VerifyCtx with the same ctx.
...
func TestPrecompile_E2E_Combine_FIPS204_VerifyCtx(t *testing.T) {
	...
	if err := VerifyCtx(params, pub, msg, precompileCtx, sig); err != nil {
		t.Fatalf("pulsarm.VerifyCtx rejected threshold-Combine signature under precompile ctx: %v", err)
	}
	...
	if err := Verify(params, pub, msg, sig); err == nil {
		t.Fatalf("pulsarm.Verify (empty ctx) accepted precompile-ctx sig — ctx is NOT propagated through Combine")
	}
}
...
func TestPrecompile_E2E_LargeCombine_FIPS204_VerifyCtx(t *testing.T) {
	...
	if err := VerifyCtx(params, groupPK, msg, precompileCtx, sig); err != nil {
		t.Fatalf("pulsarm.VerifyCtx rejected LargeCombine signature under precompile ctx: %v", err)
	}
	...
	if err := Verify(params, groupPK, msg, sig); err == nil {
		t.Fatalf("pulsarm.Verify (empty ctx) accepted precompile-ctx LargeCombine sig — ctx not propagated")
	}
}

...
var (
	ErrFieldMismatch = errors.New("pulsarm: share field mismatch with deal context")
)
...
func (LargeShamir) Reconstruct(xs []uint32, wires []LargeShareWire) ([SeedSize]byte, error) {
	if len(xs) != len(wires) {
		return [SeedSize]byte{}, errors.New("pulsarm: LargeShamir.Reconstruct: len(xs) != len(wires)")
	}
...
func (LargeShamir) ReconstructGF(xs []uint32, wires []LargeShareWire) ([SeedSize]uint32, error) {
	if len(xs) != len(wires) {
		return [SeedSize]uint32{}, errors.New("pulsarm: LargeShamir.ReconstructGF: len(xs) != len(wires)")
	}
	shares := make([]shamirShareQ, len(xs))
	for i := range xs {
		shares[i] = shareFromBytesQ(xs[i], [shareWireSizeQ]byte(wires[i]))

...
|---|---|
| **NIST MPTC submission for Pulsar** | `./SUBMISSION.md` + `NIST-SUBMISSION.md` |
| **Hanzo PQ Threshold Suite overview** | `./SUITE.md` |
| **Pulsar protocol spec** | `./SPEC.md` (text) + `./spec/pulsar.pdf` (formal) |
| **Pulsar IETF / CFRG draft** | `./docs/ietf-draft-skeleton.md` |
| **What's proved vs not proved** | `./PROOF-CLAIMS.md` |
| **Residual EC axioms** | `./AXIOM-INVENTORY.md` |
| **TCB / what to trust** | `./TRUSTED-COMPUTING-BASE.md` |
| **Op → FIPS 204 § map** | `./FIPS-TRACEABILITY.md` |
| **Patent grant** | `./PATENTS.md` |
| **Patent claim drafts (internal)** | `./docs/patent-claims.md` |
| **Per-version proof changelog** | `./CHANGELOG.md` |
| **Performance + correctness evaluation** | `./docs/evaluation.md` |
| **Magnetar (Tier 3 SLH-DSA research)** | `./docs/magnetar.md` |
...
│   ├── THRESHOLD: PULSAR (Tier 1, Hanzo suite)  ← `./`
│   └── DOCS: SUBMISSION, SPEC, IETF, PROOF, PATENTS in ./
│
├── SLH-DSA (FIPS 205)
│   ├── Single-party: FIPS 205 reference (external)
│   └── THRESHOLD: MAGNETAR (Tier 3 research)  ← `./docs/magnetar.md`
│
...
Hybrid / composition
└── HANZO PQ THRESHOLD SUITE  ← `./SUITE.md` (master index)
...
├── Entry: ./SUBMISSION.md
├── Specs: ./SPEC.md, spec/pulsar.pdf
├── Trust: PROOF-CLAIMS, AXIOM-INVENTORY, TCB
├── Validation: vectors/, docs/evaluation.md
└── IP: PATENTS.md

IETF / CFRG editor
├── Entry: ./docs/ietf-draft-skeleton.md
├── Cross-ref: ./SUITE.md (overview)
...
├── Public-facing: ./PATENTS.md
├── Claim drafts (internal): ./docs/patent-claims.md
└── Prior-art map: docs/patent-claims.md §1.4 (per claim group)

Auditor (cryptographic review)
├── Spec: ./spec/pulsar.pdf
├── Proofs: ./proofs/easycrypt/ + ~/work/lux/proofs/lean/
...
├── Reference impl: ./ref/go/pkg/pulsar/
...
- **From Axis 1 (Pulsar construction) to Axis 3 (governance)**:
  `./` is the open-source NIST-submission home; downstream
...
- **From Axis 3 (LP-019 reader) to Axis 1 (Pulsar / Magnetar)**:
  ...
  `./` for the actual NIST submission package.
...
2. **Protocol specs live in `lps/LP-NNN`** as the authoritative
   ...
   the LP-NNN.
3. **Implementation lives in the relevant repo** (`./`,
...
6. **Patent grants** live with the repo that originated the
   construction (Pulsar's grant in `./PATENTS.md`;
